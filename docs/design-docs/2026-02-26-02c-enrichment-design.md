...
yt-dlp-dj is a Chrome extension + Python local server that wraps yt-dlp for one-click music downloading with DJ-ready metadata. This design doc covers the LLM enrichment module — the component that uses the `claude` CLI to clean up messy metadata from yt-dlp into well-structured DJ-ready tags.
